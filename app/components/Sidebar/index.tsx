        ...
        <nav role="banner" className={`min-h-screen col-start-1 col-end-4 relative bg-white`}>
            <div className={`${styles.sideBar} flex flex-col gap-24`}>
                <Link href="/" className="flex justify-center">
                    <img src="../../assets/logo.svg" alt="Logo NotiApp"/>
                </Link>
                <SearchBar/>
                <ul className="mt-8 flex flex-col gap-24">
                    {
                        dataItems.map(item => 
                            <li>
                                <Link href={item.url} className={`flex items-center gap-8 ${styles.sideBarItem} ${itemSelected === item.id ? styles.sideBarItemSelected: ''}`}>
                                    <div>
                                        {item.icon}
                                    </div>
